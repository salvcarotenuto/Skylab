...
{
    public const int Nessuna=0,Visualizzazione=1,Inserimento=2,Modifica=3,InserimentoContinuativo=4,Zoom=5,Stampa=8,Login=11,Logout=12;
    ...
    public static bool IsInserimento(int azione)=>Base(azione) is Inserimento or InserimentoContinuativo;
    public static bool IsInserimentoContinuativo(int azione)=>Base(azione)==InserimentoContinuativo;
    ...
    public static bool IsReadonly(int azione)=>IsVisualizzazione(azione)||IsZoom(azione)||IsStampa(azione);
    public static int ForRecord(bool hasRecord)=>hasRecord?Modifica:Inserimento;
    public static int Normalize(int azione,int fallback)=>Base(azione) is Visualizzazione or Inserimento or Modifica or InserimentoContinuativo or Zoom or Stampa or Login or Logout?azione:fallback;
}
