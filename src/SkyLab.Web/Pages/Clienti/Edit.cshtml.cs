...
public sealed class EditModel(CustomerService service):PageModel
{
 ...
 public async Task<IActionResult> OnPostAsync(CancellationToken ct){if(!ModelState.IsValid)return Page();var id=await service.SaveCustomerAsync(Cliente,ct);TempData["Message"]="Anagrafica salvata";return RedirectToPage(new{id});}
}
