...
public sealed class CustomerService(IConfiguration configuration)
{
    ...
    public async Task<MachineEditModel?> MachineAsync(int id,CancellationToken ct){const string sql="SELECT ID,Cliente,DestinoID,Articolo,Categoria,DataRif,Valore,Durata,ProxData FROM MacchineCli WHERE ID=@id";await using var cn=new MySqlConnection(ConnectionString);await cn.OpenAsync(ct);await using var cmd=new MySqlCommand(sql,cn);cmd.Parameters.AddWithValue("@id",id);await using var r=await cmd.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct))return null;return new(){Id=r.GetInt32(0),CustomerId=r.GetInt32(1),SiteId=r.IsDBNull(2)?null:r.GetInt32(2),ArticleCode=S(r,3),CategoryId=r.IsDBNull(4)?null:r.GetInt16(4),InstalledOn=r.IsDBNull(5)?null:r.GetDateTime(5),Value=r.IsDBNull(6)?null:r.GetDecimal(6),DurationDays=r.IsDBNull(7)?null:r.GetInt16(7),NextServiceOn=r.IsDBNull(8)?null:r.GetDateTime(8)};}
