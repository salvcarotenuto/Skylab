...
public sealed class SupplierService(SkyLab.Web.Data.SkyLabDatabaseOptions options)
{
    ...
    public Task<IReadOnlyList<SupplierOption>> AccountsAsync(CancellationToken ct)=>OptionsAsync("SELECT Codice,CONCAT(LPAD(Codice,3,'0'),' - ',COALESCE(Descrizione,'')) FROM conti WHERE (Tipo='C' OR Mastro=1) AND COALESCE(Ditta,'')='F' ORDER BY Descrizione,Codice",ct);
